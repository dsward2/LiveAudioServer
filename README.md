...
A headless macOS command-line tool that reads **live 16-bit PCM audio from stdin or a UDP/TCP socket** and streams it simultaneously as **MP3** and **AAC/M4A** over HTTP — with no intermediate files, no third-party server, and no container process.
...
```swift
dependencies: [
    .package(url: "https://github.com/dsward2/LiveAudioServer.git", from: "0.1.6"),
],
targets: [
    .target(
        name: "YourHostApp",
...
Watch for upstream tools that buffer by bytes: a fixed byte buffer is fine at high sample rates but spans many chunk periods at low rates. Size the buffer by time instead:

buffer_bytes ≈ sample_rate × channels × bytes_per_sample × 0.05   # ~50 ms, safely under the 85 ms chunk period

e.g. SoX: --buffer $(( rate * channels * 2 / 20 )). (Alternatively raise --chunk-frames to tolerate burstier input, at the cost of latency and coarser filler granularity.)
